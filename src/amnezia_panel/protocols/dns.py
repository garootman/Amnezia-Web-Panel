...
"""
            ...
            self.ssh.run_sudo_command(
                "docker network ls | grep -q amnezia-dns-net || docker network create --subnet 172.29.172.0/24 amnezia-dns-net"
            )
            ...
            for c in vpn_containers:
                self.ssh.run_sudo_command(
                    f"docker ps | grep -q {c} && docker network connect amnezia-dns-net {c} || true"
                )
    ...
    def get_server_status(self, protocol_type="dns"):
        ...
        try:
            ...
            out_exists, _, _ = self.ssh.run_sudo_command(
