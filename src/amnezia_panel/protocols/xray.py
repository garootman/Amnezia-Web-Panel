...
"""
        ...
        if code_kp != 0:
            raise RuntimeError(f"Failed to generate x25519 keys: {err_kp}")
...
{self.IMAGE_NAME}"""
    ...
    def remove_container(self, protocol_type="xray"):
    ...
    def _get_meta_json(self):
        out, _, code = self.ssh.run_sudo_command("cat /opt/amnezia/xray/meta.json")
        if code != 0:
            return {}
        return json.loads(out)
    ...
    def add_client(self, protocol, client_name, server_host, port):
        ...
        clients_node.append({"id": client_id, "flow": "xtls-rprx-vision", "email": client_id})
        self._save_server_json(config)
        ...
        clients_table.append(
            {
                "clientId": client_id,
                "userData": {"clientName": client_name, "creationDate": datetime.now().isoformat(), "enabled": True},
            }
        )
        self._save_clients_table(clients_table)

        return {"client_id": client_id, "config": self.get_client_config(protocol, client_id, server_host, port)}
