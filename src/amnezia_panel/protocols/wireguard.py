...
"""
    ...
    def install_protocol(self, port=None):
        ...
        out, err, code = self.ssh.run_sudo_command(
            f"docker build --no-cache --pull -t {self.CONTAINER_NAME} {dockerfile_folder}", timeout=300
        )
        if code != 0:
...
{self.CONTAINER_NAME}"""
    ...
    def _wait_container_running(self, timeout=30):
        ...
        logs_out, _, _ = self.ssh.run_sudo_command(f"docker logs --tail 50 {self.CONTAINER_NAME} 2>&1")
        raise RuntimeError(
...
EOF
"""
        out, err, code = self.ssh.run_sudo_command(f"docker exec -i {self.CONTAINER_NAME} bash -c '{config_script}'")
        if code != 0:
...
"""
    ...
    def remove_container(self, protocol_type="wireguard"):
    ...
    def _save_clients_table(self, clients_table):
        ...
        self.ssh.run_sudo_command(f"docker cp /tmp/_wg_clients.json {self.CONTAINER_NAME}:{self.CLIENTS_TABLE_PATH}")
        self.ssh.run_command("rm -f /tmp/_wg_clients.json")

    def _get_server_config(self):
        """Get the server WireGuard config."""
        out, err, code = self.ssh.run_sudo_command(f"docker exec -i {self.CONTAINER_NAME} cat {self.CONFIG_PATH}")
        if code != 0:
    ...
    def _wg_show(self):
        ...
        out, err, code = self.ssh.run_sudo_command(f"docker exec -i {self.CONTAINER_NAME} bash -c 'wg show all'")
        if code != 0 or not out.strip():
            return {}
    ...
    def get_clients(self, protocol_type="wireguard"):
    ...
    def add_client(self, protocol_type, client_name, server_host, port=None):
        """
        Add a new client/peer to the WireGuard config.
        Returns the client config string.

        `protocol_type` and `port` are accepted for uniform dispatch and ignored —
        WireGuard has one protocol variant and derives its listen port from the server config.
        """
...
"""
    ...
    def get_client_config(self, protocol_type, client_id, server_host, port=None):
...
"""
            ...
            self.ssh.run_sudo_command(f"docker cp /tmp/_wg_config.conf {self.CONTAINER_NAME}:{self.CONFIG_PATH}")
    ...
    def get_server_status(self, protocol_type="wireguard"):
