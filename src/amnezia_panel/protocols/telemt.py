...
class TelemtManager:
    ...
    def install_protocol(
        self, protocol_type="telemt", port="443", tls_emulation=True, tls_domain="", max_connections=0
    ):
        results = []
        ...
        with open(os.path.join(local_dir, "config.toml"), encoding="utf-8") as f:
            config_content = f.read()

        tls_emul_str = "true" if tls_emulation else "false"
        config_content = re.sub(
            r"tls_emulation\s*=\s*(true|false|True|False)", f"tls_emulation = {tls_emul_str}", config_content
        )
        ...
        with open(os.path.join(local_dir, "docker-compose.yml"), encoding="utf-8") as f:
        ...
        # Upload Dockerfile
        with open(os.path.join(local_dir, "Dockerfile"), encoding="utf-8") as f:
            dockerfile = f.read()
            self.ssh.upload_file_sudo(dockerfile, f"{remote_dir}/Dockerfile")

        results.append("Starting Telemt container...")
        out, err, code = self.ssh.run_sudo_command(
            f"sh -c 'cd {remote_dir} && docker compose up -d --build'", timeout=600
        )
        if code != 0:
